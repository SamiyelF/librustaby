...
int create_connect_once() { return Create::instance()->connect() ? 1 : 0; }

void create_disconnect() { Create::instance()->disconnect(); }

void create_passive() { Create::instance()->setPassiveMode(); }

void create_safe() { Create::instance()->setSafeMode(); }

void create_full() { Create::instance()->setFullMode(); }

int get_create_mode() { return Create::instance()->mode(); }
...
int get_create_wall() { return Create::instance()->wall()->value() ? 1 : 0; }
...
int get_create_vwall() { return Create::instance()->virtualWall()->value(); }

int get_create_infrared() { return Create::instance()->ir()->value(); }
...
void set_create_normalized_angle(int angle) { set_create_total_angle(angle); }

int get_create_total_angle() { return Create::instance()->angle()->value(); }

void set_create_total_angle(int angle) { Create::instance()->setAngle(angle); }

int get_create_distance() { return Create::instance()->distance()->value(); }

void set_create_distance(int dist) { Create::instance()->setDistance(dist); }
...
int get_create_wall_amt() { return Create::instance()->wallSignal()->value(); }
...
void set_create_baud_rate(const enum BaudRate baudRate) {
  Create::instance()->setDefaultBaudRate(
      static_cast<Create::BaudRate>(baudRate));
}
...
void create_stop() { Create::instance()->stop(); }
...
void create_spin_CW(int speed) { Create::instance()->spinClockwise(speed); }
...
int _create_get_raw_encoders(long *lenc, long *renc) { return 0; }

int create_load_song(const unsigned char *song, const unsigned char length,
...
int create_read_block(char *data, int count) {
  return Create::instance()->blockingRead(
      reinterpret_cast<unsigned char *>(data), count, UINT_MAX);
}
...
void create_clear_serial_buffer() { Create::instance()->flush(); }
