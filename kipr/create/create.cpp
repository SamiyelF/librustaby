...
EM_JS(short, jsCreateRead, (), { return Module.context.createRead(); });
} // namespace
#endif
...
template <typename T> T *construct(T *dummy, const Create *parent) {
...
const static unsigned int baudCodeRate[12] = {300,   600,   1200,  2400,
...
public:
  ...
  virtual bool value() const { return m_create->sensorPacket1()->wall; }
...
public:
  ...
  virtual bool value() const { return m_create->sensorPacket1()->cliffLeft; }
...
public:
  ...
  virtual bool value() const { return m_create->sensorPacket1()->cliffRight; }
...
public:
  ...
  virtual bool value() const { return m_create->sensorPacket1()->virtualWall; }
...
public:
  ...
  virtual unsigned char value() const { return m_create->sensorPacket2()->ir; }
...
CreateScript::CreateScript(const CreateScript &rhs) { append(rhs); }

void CreateScript::append(const unsigned char &byte) { append(&byte, 1); }
...
size_t CreateScript::size() const { return m_script.size(); }

unsigned char CreateScript::byte(const size_t &i) { return m_script[i]; }
...
bool Create::connect() { return connect(m_defaultBaudRate); }
...
bool Create::connect(const Create::BaudRate baudRate) {
  ...
  if (!open())
    return false;
#ifndef WIN32
  static int rates[2] = {B57600, B115200};
  if (set_interface_attribs(m_tty, rates[static_cast<unsigned>(baudRate)], 0) !=
      0) {
    close();
    return false;
  }
...
bool Create::isConnected() const { return m_tty; }
...
void Create::send(const CreateScript &script) { m_script = script; }

bool Create::write(const unsigned char &c) { return write(&c, 1); }
...
void Create::setLeds(const bool &advance, const bool &play,
...
void Create::driveStraight(const short &speed) { driveDirect(speed, speed); }
...
void Create::spinClockwise(const short &speed) { spin(-speed); }

void Create::spinCounterClockwise(const short &speed) { spin(speed); }
...
bool Create::loadSong(const unsigned char *song, const unsigned char length,
                      const unsigned char songNum) {
  if (!isConnected()) {
    std::cout << "Not connected. Please connect before trying to load a song."
              << std::endl;
    return false;
  }
...
bool Create::playSong(const unsigned char songNum) {
  if (!isConnected()) {
    std::cout << "Not connected. Please connect before trying to play a song."
              << std::endl;
    return false;
  }
...
const CreateState *Create::state() { return &m_state; }
...
button::AbstractButton *Create::advanceButton() const
    LAZY_RETURN(m_advanceButton);
...
const unsigned short &Create::refreshRate() const { return m_refreshRate; }
...
Create::Create()
    : m_refreshRate(10), m_playButton(0), m_advanceButton(0), m_wall(0),
      m_cliffLeft(0), m_cliffFrontLeft(0), m_cliffFrontRight(0),
      m_cliffRight(0), m_virtualWall(0), m_wallSignal(0), m_cliffLeftSignal(0),
      m_cliffFrontLeftSignal(0), m_cliffFrontRightSignal(0),
      m_cliffRightSignal(0), m_lightBumpLeft(0), m_lightBumpFrontLeft(0),
      m_lightBumpFrontRight(0), m_lightBumpCenterLeft(0),
      m_lightBumpCenterRight(0), m_lightBumpRight(0), m_lightBumpLeftSignal(0),
      m_lightBumpFrontLeftSignal(0), m_lightBumpFrontRightSignal(0),
      m_lightBumpCenterLeftSignal(0), m_lightBumpCenterRightSignal(0),
      m_lightBumpRightSignal(0), m_cargoBayAnalogSignal(0),
      m_cargoBayDigitalInputs(0), m_ir(0), m_chargingState(0),
      m_batteryTemperature(0), m_batteryCharge(0), m_batteryCapacity(0),
      m_angle(0), m_distance(0), m_bumpLeft(0), m_bumpRight(0),
      m_wheelDropLeft(0), m_wheelDropRight(0), m_wheelDropCaster(0),
      m_defaultBaudRate(Baud115200), m_songPlaying(0), m_songNumber(0) {
#ifndef WIN32
...
void Create::updateState() { m_state.timestamp = timeOfDay(); }
