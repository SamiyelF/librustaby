...
void shut_down_in(double s) {
  ...
  if (s_instance) {
    std::cout << "Warning: shut_down_in already called once. Ignoring."
              << std::endl;
    return;
  }
...
/* Assumption is that if light is detected by consecutive readings FLASHPAUSE
   apart, it's the start light, not a flash NOTE: when tested with camera, the
   flash persisted < 0.1 sec
*/
...
#define DJUMP                                                                  \
  2 // how far to jump to next red circle - affects speed of red circle
...
void wait_for_light(int light_port) {
  ...
  while (!OK) {
    ...
    if ((offval - onval) >= THRESHOLD) { // bright = small values
      ...
      graphics_rectangle_fill(INSET, CROWS / 2 - 20, CCOLS - INSET,
                              CROWS / 2 + 20, GREEN);
      ...
      while (1) {             // loop until light verified on
        ...
        graphics_rectangle_fill(INSET + 210, 30, INSET + 270, 60,
                                WHITE); // erase text
        graphics_print_int(reading - onval, 1, INSET + 210, 30, BLACK, 2);
        if ((reading - onval) <
            THRESHOLD) {                // reading is low enough for light on
...
void colorbar(int i, int range) {
  ...
  d = i % (int)(2 * limit); // scale i to total number of jumps (half forward
                            // and half back)
  if (d < limit)
    graphics_circle_fill(INSET + DJUMP * d, CROWS / 2, 20,
                         RED); // move to right
  else
    graphics_circle_fill(INSET + DJUMP * limit - DJUMP * (d - limit), CROWS / 2,
                         20, RED); // move to left
  graphics_update();
  if (d < limit)
    graphics_circle_fill(INSET + DJUMP * d, CROWS / 2, 20,
                         GREEN); // remove circle from next iteration
  else
    graphics_circle_fill(INSET + DJUMP * limit - DJUMP * (d - limit), CROWS / 2,
                         20, GREEN);
}
