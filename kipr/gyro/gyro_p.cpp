...
namespace {
// Biases default to zero so that the low pass only takes affect if calibration
// is run.
int biasx = 0;
int biasy = 0;
int biasz = 0;
...
// Pulls gyroscope data from the I2C registers. The MPU 9250 outputs high and
// low registers that need to be combined.
short kipr::gyro::gyro_x() {
  return static_cast<signed short>(
             Platform::instance()->readRegister16b(REG_RW_GYRO_X_H)) /
             16 -
         biasx;
}
...
short kipr::gyro::gyro_y() {
  return static_cast<signed short>(
             Platform::instance()->readRegister16b(REG_RW_GYRO_Y_H)) /
             16 -
         biasy;
}
...
short kipr::gyro::gyro_z() {
  return static_cast<signed short>(
             Platform::instance()->readRegister16b(REG_RW_GYRO_Z_H)) /
             16 -
         biasz;
}
...
bool kipr::gyro::gyro_calibrate() {
  ...
  // Find the average noise of the gyro. Get the bias for gyro axis by sampling
