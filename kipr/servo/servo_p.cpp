...
void kipr::servo::set_servo_enabled(int port, bool enabled) {
  ...
  unsigned short allStop =
      Platform::instance()->readRegister8b(REG_RW_MOT_SRV_ALLSTOP);
...
bool kipr::servo::set_servo_position(int port, unsigned short position) {
  ...
  unsigned short val =
      1500 + std::round(1800.0 * ((double)position / 2047.0)) - (1800 / 2);
...
unsigned short kipr::servo::get_servo_position(int port) {
  ...
  const unsigned short position =
      Platform::instance()->readRegister16b(address);
