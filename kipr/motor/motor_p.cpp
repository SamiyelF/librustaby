...
int per_tick_small_to_large(int val) { return val * update_Hz; }

int per_tick_large_to_small(int val) { return val / update_Hz; }

// TODO: this is a hack but the hardware timers are making it hard to easily
// swap motors 2,3 today
...
unsigned char kipr::motor::get_motor_mode(unsigned int port) {
  if (port < 0 || port > 3)
    return static_cast<unsigned char>(
        ControlMode::Inactive); // TODO: better fail code
...
bool kipr::motor::clear_motor_bemf(unsigned int port) {
  if (port >= NUM_MOTORS)
    return false;
  Platform::instance()->writeRegister32b(REG_RW_MOT_0_B3 + 4 * fix_port(port),
                                         0);
  return true;
}
...
int kipr::motor::get_motor_bemf(unsigned int port) {
  if (port >= NUM_MOTORS)
    return 0;
  int val = Platform::instance()->readRegister32b(REG_RW_MOT_0_B3 +
                                                  4 * fix_port(port));
...
int kipr::motor::get_motor_bemf_vel(unsigned int port) {
  if (port >= NUM_MOTORS)
    return 0;
  int val = Platform::instance()->readRegister32b(REG_RW_MOT_0_SP_H +
                                                  4 * fix_port(port));
  return val;
}
...
unsigned char kipr::motor::get_motor_direction(unsigned int port) {
  if (port >= NUM_MOTORS)
    return static_cast<unsigned char>(Direction::PassiveStop);
...
unsigned int kipr::motor::get_motor_pwm(unsigned int port) {
  if (port >= NUM_MOTORS)
    return 0;
  // TODO: error signal outside of range
  return Platform::instance()->readRegister16b(REG_RW_MOT_0_PWM_H +
                                               2 * fix_port(port));
}
...
bool kipr::motor::set_motor_pwm(unsigned int port, unsigned char speed) {
  ...
  if (adjustedSpeed > speedMax)
    adjustedSpeed = speedMax; // TODO: check scaling (1/4 percent increments)
  Platform::instance()->writeRegister16b(
      REG_RW_MOT_0_PWM_H + 2 * fix_port(port), adjustedSpeed);
...
bool kipr::motor::set_motor_goal_velocity(unsigned int port,
                                          int goal_velocity) {
  ...
  // TODO: may need to put some logic in for not writing goals if they equal the
  // current goal
  //  ... maybe add on the co-proc?
  unsigned int goal_addy =
      REG_RW_MOT_0_SP_H + 2 * fix_port(port); // TODO: 32 bit?
  std::lock_guard<std::mutex> lock(cleanup_mutex);
  Platform::instance()->writeRegister16b(
      goal_addy, static_cast<signed short>(goal_velocity));
...
bool kipr::motor::set_motor_goal_position(unsigned int port,
                                          int goal_position) {
  if (port >= NUM_MOTORS)
    return false;

  // TODO
  // TODO: logic to not set goals if they match the current value (in co-proc
  // firmware maybe?)
  unsigned int goal_addy = REG_W_MOT_0_GOAL_B3 + 4 * fix_port(port);
  Platform::instance()->writeRegister32b(
      goal_addy, per_tick_small_to_large(goal_position));
...
bool kipr::motor::set_motor_pid_gains(int port, short p, short i, short d,
                                      short pd, short id, short dd) {
  if (port >= NUM_MOTORS)
    return false;

  unsigned int addy_offset =
      12 * fix_port(port); // 6 registers at 2 bytes each... per port
...
bool kipr::motor::get_motor_pid_gains(int port, short &p, short &i, short &d,
                                      short &pd, short &id, short &dd) {
  if (port >= NUM_MOTORS)
    return false;
...
bool kipr::motor::get_motor_stop(unsigned int port) {
  ...
  bool stopped = get_motor_mode(port) ==
                     static_cast<unsigned char>(ControlMode::Inactive) &&
                 get_motor_direction(port) ==
                     static_cast<unsigned char>(Direction::PassiveStop);
  return stopped;
}
...
bool kipr::motor::get_motor_done(unsigned int port) {
  if (port >= NUM_MOTORS)
    return false;
  unsigned char motors_done =
      Platform::instance()->readRegister8b(REG_RW_MOT_DONE);
