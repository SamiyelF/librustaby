...
int get_motor_position_counter(int motor) { return get_motor_bemf(motor); }

int gmpc(int motor) { return get_motor_position_counter(motor); }

void clear_motor_position_counter(int motor) { motor::clear_motor_bemf(motor); }

void cmpc(int motor) { clear_motor_position_counter(motor); }
...
void set_pid_gains(int motor, short p, short i, short d, short pd, short id,
                   short dd) {
...
void bmd(int motor) { block_motor_done(motor); }
...
int getpwm(int motor) { return -1; }

void fd(int motor) { ::motor(motor, 100); }

void bk(int motor) { ::motor(motor, -100); }
...
void motor_power(int motor, int percent) {
  ...
  if (percent > 0) {
    ...
    set_motor_direction(motor,
                        static_cast<unsigned char>(Direction::PassiveStop));
  }
}

void off(int motor) { stop_motor(motor); }
