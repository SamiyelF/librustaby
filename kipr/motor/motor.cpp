...
void Motor::clearPositionCounter() { motor::clear_motor_bemf(m_port); }
...
void Motor::moveToPosition(short speed, int goalPos) {
  ...
  motor::set_motor_mode(m_port,
                        static_cast<unsigned char>(ControlMode::SpeedPosition));
...
void Motor::setPidGains(short p, short i, short d, short pd, short id,
                        short dd) {
  motor::set_motor_pid_gains(m_port, p, i, d, pd, id, dd);
}

void Motor::pidGains(short &p, short &i, short &d, short &pd, short &id,
                     short &dd) {
...
void Motor::freeze() {
  ...
  motor::set_motor_direction(m_port,
                             static_cast<unsigned char>(Direction::ActiveStop));
}
...
void Motor::forward() { motor(100); }

void Motor::backward() { motor(-100); }
...
void Motor::motorPower(int percent) {
  ...
  if (percent > 0) {
    motor::set_motor_direction(m_port,
                               static_cast<unsigned char>(Direction::Forward));
  } else if (percent < 0) {
    motor::set_motor_direction(m_port,
                               static_cast<unsigned char>(Direction::Reverse));
  } else {
    motor::set_motor_direction(
        m_port, static_cast<unsigned char>(Direction::PassiveStop));
  }
}

void Motor::off() { motor::stop_motor(m_port); }

int Motor::port() const { return m_port; }

BackEMF::BackEMF(int port) : m_port(port) {}

int BackEMF::value() const { return motor::get_motor_bemf(m_port); }

int BackEMF::port() const { return m_port; }
