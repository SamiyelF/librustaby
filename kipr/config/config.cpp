...
void Config::clearGroup() { m_groups.clear(); }
...
bool Config::boolValue(const std::string &key) const { return intValue(key); }
...
std::string Config::stringValue(const std::string &key) const {
  std::map<std::string, std::string>::const_iterator cit =
      m_config.find(group() + safeKey(key));
  // std::cout << "Value for " << key << " = \"" << cit->second << "\"" <<
  // std::endl; Invalid key
  if (cit == m_config.end())
    return "";
  return cit->second;
}
...
Config Config::values() const {
  ...
  for (; it != m_config.end(); ++it) {
    ...
    if (key.substr(0, group.size()).compare(group))
      continue;
...
void Config::addValues(const Config &config) {
  ...
  std::map<std::string, std::string>::const_iterator it =
      values.m_config.begin();
  for (; it != values.m_config.end(); ++it)
