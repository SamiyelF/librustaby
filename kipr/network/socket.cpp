...
Address::Address(const sockaddr_in &addr) : m_valid(true), m_addr(addr) {}

Address::Address() : m_valid(false) {}

bool Address::isValid() const { return m_valid; }
...
unsigned short Address::port() const { return ntohs(m_addr.sin_port); }
...
socklen_t Address::addrLength() const { return sizeof(m_addr); }

const char *Address::ip() const { return inet_ntoa(m_addr.sin_addr); }
...
bool Socket::isOpen() const { return m_fd >= 0; }
...
socket_fd_t Socket::fd() const { return m_fd; }
...
ssize_t Socket::recvfrom(void *const buffer, const size_t length,
                         Address &address, int flags) {
  sockaddr_in rawAddress;
  socklen_t rawAddressLength;
  ssize_t ret = ::recvfrom(m_fd, reinterpret_cast<char *>(buffer), length,
                           flags, (sockaddr *)&rawAddress, &rawAddressLength);
...
ssize_t Socket::sendto(const void *const buffer, const size_t length,
                       const Address &dest, int flags) {
  return ::sendto(m_fd, reinterpret_cast<const char *>(buffer), length, 0,
                  dest.addr(), dest.addrLength());
}
