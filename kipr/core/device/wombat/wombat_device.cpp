...
public:
  WombatDevice()
      : spi_fd_(-1), count(0), read_buf(new std::uint8_t[REG_READABLE_COUNT]),
  ...
  virtual std::uint32_t r32(const std::uint8_t address) override {
    ...
    return (read_buf[address] << 24 | read_buf[address + 1] << 16 |
            read_buf[address + 2] << 8 | read_buf[address + 3] << 0);
  }

  virtual void w8(const std::uint8_t address,
                  const std::uint8_t value) override {
    std::lock_guard<std::mutex> lock(mut_);
    clear_buffers();
    write_buf[3] = 1, write_buf[4] = address, write_buf[5] = value;

    transfer();
  }

  virtual void w16(const std::uint8_t address,
                   const std::uint16_t value) override {
    std::lock_guard<std::mutex> lock(mut_);
    clear_buffers();
    write_buf[3] = 2, write_buf[4] = address,
    write_buf[5] = (value & 0xFF00) >> 8, write_buf[6] = address + 1,
    write_buf[7] = (value & 0x00FF) >> 0,

    transfer();
  }

  virtual void w32(const std::uint8_t address,
                   const std::uint32_t value) override {
    std::lock_guard<std::mutex> lock(mut_);
    clear_buffers();
    write_buf[3] = 4;
    write_buf[4] = address, write_buf[5] = (value & 0xFF000000) >> 24,
    write_buf[6] = address + 1, write_buf[7] = (value & 0x00FF0000) >> 16,
    write_buf[8] = address + 2, write_buf[9] = (value & 0x0000FF00) >> 8,
    write_buf[10] = address + 3, write_buf[11] = (value & 0x000000FF) >> 0,
...
private:
  ...
  bool transfer() {
    ...
    usleep(50); // FIXME: this  makes sure we don't outrun the co-processor
                // until interrupts are in place for DMA
...
struct WombatDeviceDescriptor {
  ...
  static bool isPresent() { return access(SPI_FILE_SYSTEM, F_OK) == 0; }
};
