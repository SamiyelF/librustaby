...
public:
  EmscriptenFsDevice()
      : fd_(-1), write_buffer_(nullptr), write_buffer_size_(0) {
  ...
  virtual ~EmscriptenFsDevice() { delete[] write_buffer_; }
  ...
  virtual void submit(const Command *const buffer,
    ...
    for (std::size_t i = 0; i < size; ++i) {
      const Command &command = buffer[i];
      if (command.type != Command::Type::Read)
        continue;
      ...
      case 2: {
        *command.value =
            ((work_buffer[in_iter + 0] << 8) | (work_buffer[in_iter + 1] << 0));
        in_iter += 2;
        break;
      }
      case 4: {
        *command.value =
            ((work_buffer[in_iter + 0] << 24) |
             ...
             (work_buffer[in_iter + 2] << 8) | (work_buffer[in_iter + 3] << 0));
        in_iter += 4;
        break;
      }
      }
...
private:
  bool read(const std::uint8_t address, std::uint8_t *const values,
  ...
  bool write(const std::uint8_t address, const std::uint8_t *const values,
             const std::uint8_t count) {
