...
// Full screen management functions for the display window as implemented on the
// KIPR LINK controller
//   display_printf(int column, int row, <printf arguments>)
//
// Console screen display window size is 10 rows and limited to 42 columns,
// indexed from 0
//
...
#define __VBUTTONS                                                             \
  2 // set to 0 if A,B,C virtual buttons are not going to be implemented
// Copyright(c) KIPR, 2013
// Full screen management functions for the display window as implemented on the
// KIPR LINK controller
//   display_printf(int column, int row, <printf arguments>)
//
// Link Console screen display window size is 10 rows and limited to 42 columns,
// indexed from 0 Wallaby console is 13 rows and limited to 57 columns
//
// Initial version: 1/8/2013 - cnw
...
//    reconfigured for Wallaby controller - virtual buttons not in at time,
//    which may require a further adjustment
// stdarg.h provides macros for accessing a function's argument list ... see K&R
// Revision: 8/6/2019 - cnw
//    reconfigured for Wombat KRC controller - reduced line count and column
//    count to conform to Wombat KRC UI display
...
#define _MAPy                                                                  \
  11 // 9 rows when 3 buttons visible, loses 2 rows for each line of virtual
     // buttons
#define _MAPx                                                                  \
  43 // 41 columns is max fit for screen (array has 42 to accommodate \0)
...
void display_printf(int col, int row, const char *t,
  ...
  int rb6 = __VBUTTONS;    // row adjustment (default is 2, if there are no
                           // buttons, change to 0)
  char *dp, ws[256]; // pointer into display map, working string for vsprintf

  va_start(argp,
           t); // t is last named argument in display_printf's function header;
  ...
  vsprintf(ws, t,
           argp); // same as sprintf except requires a pointer to argument list
