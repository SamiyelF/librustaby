...
struct Graphics {
  ...
  inline int width() const { return display.width(); }

  inline int height() const { return display.height(); }
...
public:
  PixelToasterListener() { memset(m_keyStates, false, sizeof(bool) * 0xFF); }

  bool isMiddleButtonDown() const { return m_mouse.buttons.middle; }

  bool isLeftButtonDown() const { return m_mouse.buttons.left; }

  bool isRightButtonDown() const { return m_mouse.buttons.right; }

  int mouseX() const { return m_mouse.x; }

  int mouseY() const { return m_mouse.y; }

  bool isKeyPressed(const int key) { return m_keyStates[key]; }
...
protected:
  ...
  void onMouseMove(DisplayInterface &display, Mouse mouse) { m_mouse = mouse; }
...
inline Pixel fromTrueColor(Encoding enc, const int _0, const int _1,
                           const int _2) {
  switch (enc) {
  case RGB:
    return Pixel(_0 / 255.0, _1 / 255.0, _2 / 255.0);
  case BGR:
    return Pixel(_2 / 255.0, _1 / 255.0, _0 / 255.0);
  }
}

int graphics_open(int width, int height) {
  if (!g_graphics.display.open("KISS Graphics Window", width, height))
    return false;
...
void graphics_close() { g_graphics.display.close(); }
...
void graphics_clear() { graphics_fill(0, 0, 0); }

void graphics_blit(const unsigned char *data, int x, int y, int width,
                   int height) {
  graphics_blit_enc(data, RGB, x, y, width, height);
}

void graphics_blit_region(const unsigned char *data, int sx, int sy, int ex,
                          int ey, int width, int height, int dx, int dy) {
  graphics_blit_region_enc(data, RGB, sx, sy, ex, ey, width, height, dx, dy);
}

void graphics_blit_enc(const unsigned char *data, Encoding enc, int x, int y,
                       int width, int height) {
  graphics_blit_region_enc(data, enc, 0, 0, width - 1, height - 1, width,
                           height, x, y);
}

void graphics_blit_section(const unsigned char *data, Encoding enc,
                           const int index, const int dindex,
                           const int length) {
  int actualLength =
      length + dindex < g_graphics.size() ? length : g_graphics.size() - dindex;

  if (actualLength > g_graphics.size())
    actualLength = g_graphics.size();
  if (actualLength <= 0)
    return;

  for (int i = 0; i < actualLength; ++i) {
    const unsigned int offset = (i + index) * 3;
    g_graphics.pixels[dindex + i] =
        fromTrueColor(enc, data[offset], data[offset + 1], data[offset + 2]);
  }
}

void graphics_blit_region_enc(const unsigned char *data, Encoding enc, int sx,
                              int sy, int ex, int ey, int width, int height,
                              int dx, int dy) {
  if (sx >= ex || sy >= ey)
    return;
  if (dx >= g_graphics.width() || dy >= g_graphics.height())
    return;
  ...
  for (int i = 0; i < hsize; ++i) {
    int cols = ex - sx + 1;
    if (cols + dx > g_graphics.width())
      cols -= dx;
    const int index = (sy + i) * width + sx;
    const int dindex = (dy + i) * g_graphics.width() + dx;
    graphics_blit_section(data, enc, index < 0 ? 0 : index,
                          dindex < 0 ? 0 : dindex, cols);
  }
}
...
void graphics_pixel(int x, int y, int r, int g, int b) {
  if (!g_graphics.isValidPoint(x, y))
    return;
...
void graphics_rectangle_fill(int x1, int y1, int x2, int y2, int r, int g,
                             int b) {
  if (x1 > x2)
    swap(x1, x2);
  if (y1 > y2)
...
void graphics_triangle(int x1, int y1, int x2, int y2, int x3, int y3, int r,
                       int g, int b) {
...
void graphics_triangle_fill(int x1, int y1, int x2, int y2, int x3, int y3,
                            int r, int g, int b) {
...
int get_mouse_middle_button() { return g_listener.isMiddleButtonDown(); }

int get_mouse_left_button() { return g_listener.isLeftButtonDown(); }

int get_mouse_right_button() { return g_listener.isRightButtonDown(); }
