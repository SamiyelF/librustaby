...
Compass::Compass()
    : mean_x(0.f), mean_y(0.f), mean_z(0.f), w1(0.f), w2(0.f), div_e1(0.f),
      div_e2(0.f) {}
...
/**
   ...
   two full turns during the calibration process.  The turning should be a
   consistent speed throughout to make sure that we achieve a regular sampling.

   Theory of operation:
   - The magnetometer vector samples form an ellipse in 3 dimensions.  Because
   the wallaby is sitting flat (or nearly flat), this ellipse is almost
   horizontal.  Our goal is to compute the parameters that are necessary to
   transform this ellipse into a circle that is sitting in the x/y plane
...
void Compass::calibrate() {
  printf("Slowly turn the compass for two full cycles, keeping the compass "
         "flat...\n\r");
  msleep(2000);
  ...
  div_e1 = sqrt(1 + w1 * w1);
  div_e2 =
      sqrt(w1 * w1 * w2 * w2 + 1.0 + 2 * w1 * w1 + w1 * w1 * w1 * w1 + w2 * w2);

  // Give the user the line of code that will calibrate the compass directly
  printf("Complete: set_compass_params(%f, %f, %f, %f, %f, %f, %f)\n\r", mean_x,
         mean_y, mean_z, w1, w2, div_e1, div_e2);
}

void Compass::setParams(float meanX, float meanY, float meanZ, float W1,
                        float W2, float div_E1, float div_E2) {
  mean_x = meanX;
  mean_y = meanY;
  mean_z = meanZ;
  w1 = W1;
...
float Compass::getAngle(void) {
  ...
  float p_y =
      (-mag[0] * w1 * w2 + mag[1] * (1 + w1 * w1) + mag[2] * w2) / div_e2;
